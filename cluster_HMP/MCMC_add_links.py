...
i_train = np.array(range(metabolome.shape[1])) #np.sort(np.random.choice(range(metabolome.shape[1]), int(metabolome.shape[1]*0.75), replace=False))
...
for i in range(10000):
    if i%50==0:
        print(i)
    if np.random.uniform(0,1,1)[0] <= 1.0:
